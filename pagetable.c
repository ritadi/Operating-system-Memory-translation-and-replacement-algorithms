...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
        ...
        //find the pte of frame
        pgtbl_entry_t *victim = coremap[frame].pte;
        
        if (victim->frame & PG_DIRTY){
            
            evict_dirty_count++;
            //put page into swapfile
            int swap_offset;
            swap_offset = swap_pageout(victim->frame >> PAGE_SHIFT, victim->swap_off);
            //swap_pageout contains error checking
            //update pte offset
        ...
        //mark frame as on_swap because not in mem anymore
...
char *find_physpage(addr_t vaddr, char type) {
    ...
    pgtbl_entry_t *entry = (pgtbl_entry_t *)(pgdir[idx].pde & PAGE_MASK);

	// Use vaddr to get index into 2nd-level page table and initialize 'p'

    p = &entry[PGTBL_INDEX(vaddr)];
    ...
    if (!(p->frame & PG_VALID)){
        ...
        if (!(p->frame & PG_ONSWAP)){
            //allocate a frame for virtual page p
            allo_frame = allocate_frame(p);
            
            init_frame(allo_frame,vaddr);
            //store PFN
            p->frame = allo_frame << PAGE_SHIFT;
            //set to dirty page
            p->frame |= PG_DIRTY;
    ...
    //accomodate for opt
	coremap[allo_frame].vaddr = (vaddr >> PAGE_SHIFT) << PAGE_SHIFT; 
